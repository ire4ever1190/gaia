...
public:
    ...
    /// Processes a list of message content blocks (text + images).
    /// This allows sending multiple messages or different message types (e.g. images).
    ///
    /// @param contents Series of content blocks to send to the agent
    /// @param maxSteps Override max steps (0 = use config default)
    /// @return JSON result with "result" key containing the final answer
